        ...
        <i-content :style="{padding: '24px', minHeight: '428px', background: '#fff'}">
          <div style="width:100%;">
            <div style="width: 600px;margin-left:  auto;margin-right:  auto;">
            	<i-form method="post" action="<%=ctxPath %>/jb/worktime/updaterecord" ref="formValidate" :model="formParm" :rules="ruleValidate" :label-width="80">
                	<input type="hidden" name="id" v-model="formParm.id"/>
                	...
                	<form-item label="加班时间" prop="workdate">
                  		<Date-picker  :readonly="true" name="workDate" type="date" v-model="formParm.workdate"  format="yyyy-MM-dd"  placeholder="加班日期"  >
                    	</Date-picker>  
                	</form-item >
                  ...
                  <form-item v-for="(item, index) in items" :key="index" >
                    <Row>
                      <i-col span="8">
                        <time-picker confirm :editable="false" name="startTime" :value="item.startTime" v-model="item.startTime" :steps="[1, 30]" format="HH:mm" placeholder="开始时间段" style="width: 150px"></time-picker>
                      </i-col>
                      <i-col span="1" >
                        至
                      </i-col>
                      <i-col span="8">
                      	<Tooltip content="结束时间为24:00时，请选择00:00" placement="right-start">
                        	<time-picker confirm :editable="false" name="endTime" :value="item.endTime" v-model="item.endTime" :steps="[1, 30]" format="HH:mm" placeholder="结束时间段" style="width: 150px"></time-picker>
                        </Tooltip>
                      </i-col>
...
var params = <%=request.getAttribute("recordJson") %> ;
var myVue =  new Vue({
	  el: '#app_content',
	  data:{
		  formParm:{
		   ...
		   handleSubmit:function (name) {
	            this.$refs[name].validate((valid) => {
	                if (valid) {
	                	...
	                	param.append("id",myVue.formParm.id); 
